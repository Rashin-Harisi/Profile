...
export const BentoGridItem = ({
    ...
    const leftList = ["TypeScript", "ReactJS" , "NextJS" ];
    const rightList = ["MongoDB", "GraphQl", "Tailwind"];

    const [copied, setCopied] = useState(false)

    const handleClick =()=>{
        navigator.clipboard.writeText("rashin.harisi@gmail.com")
        setCopied(true)
    }
    return (
        <div
        ...
        >
            <div className={`${id === 6 && "flex justify-center"} h-full`}>
                ...
                <div className={`absolute right-0 -bottom-5 ${id === 5 && "w-full opacity-80"}`}>
                    {spareImg && (<img src={spareImg} alt={spareImg}
                ...
                text-3xl text-center md:text-4xl lg:text-7xl"/>*/}
                ...
                <div className={cn(titleClassName, "group-hover/bento:translate-x-2 transition duration-200 relative md:h-full min-h-40 flex flex-col px-5 p-5 lg:p-10")}>
                    ...
                    <div className="font-sans font-bold text-lg 3lg:text-3xl max-w-96 z-10">
                        {title}
                    </div>
                {id === 2 && <GlobeDemo />}
                {id === 3 && (
